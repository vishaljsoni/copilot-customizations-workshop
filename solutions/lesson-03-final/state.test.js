...
test("filterTodos returns all by default", () => {
  const a = createTodo("a", 1);
  const b = { ...createTodo("b", 2), completed: true };
  assert.equal(filterTodos([a, b], "all").length, 2);
});

test("filterTodos active and completed", () => {
  const a = createTodo("a", 1);
  const b = { ...createTodo("b", 2), completed: true };
  assert.deepEqual(
    filterTodos([a, b], "active").map((t) => t.text),
    ["a"]
  );
  assert.deepEqual(
    filterTodos([a, b], "completed").map((t) => t.text),
    ["b"]
  );
});
